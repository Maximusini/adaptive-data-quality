...
def train():
    ...
    train_data_path = settings.BASE_DIR / 'training_dataset'
    ...
    df['temp_std'] = df.groupby('sensor_id')['temperature'].transform(lambda x: x.rolling(window=settings.WINDOW_SIZE).std())
    df['hum_std'] = df.groupby('sensor_id')['humidity'].transform(lambda x: x.rolling(window=settings.WINDOW_SIZE).std())
    ...
    # NaN из-за сдвига
    df = df.dropna(subset=['temp_std', 'hum_std'])
    
    df['is_frozen'] = (df['temp_std'] <= 0.001) | (df['hum_std'] <= 0.001)
    df_for_ml = df[df['is_frozen'] == False].copy()

    X = df_for_ml[settings.FEATURE_COLUMNS]
    y = df_for_ml['is_anomaly'].astype(int)
    
    logger.info(f'Training data size: {len(X)} samples')
    ...
    importances = pd.Series(rf.feature_importances_, index=settings.FEATURE_COLUMNS).sort_values(ascending=False)
    print('\nТоп признаков')
    print(importances)

    joblib.dump(rf, settings.MODEL_PATH)
